...
from common_modules.common_objects import bot, dp
...
async def create_fsm_acc_change(message: types.Message):
    ...
    await message.answer("Выбери дату:", reply_markup=keyboards.kb_date)
...
async def choose_date(message: types.Message, state: FSMContext):
    ...
    if message.text == 'Сегодня':
        ...
        await bot.send_message(message.from_user.id, 'Что будем вносить?', reply_markup=keyboards.kb_acc_change)
...
async def process_dialog_calendar(callback_query: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    if selected:
        ...
        async with state.proxy() as data:
            data['date'] = date
            await callback_query.message.answer('Что будем вносить?', reply_markup=keyboards.kb_acc_change)
...
async def set_amount(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(message.from_user.id, 'Что дальше?', reply_markup=keyboards.kb_end_acc_change)
